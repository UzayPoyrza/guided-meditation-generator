#!/usr/bin/env python3
"""
Batch meditation TTS processor — Luna voice variant (_l suffix).

Usage:
    python run_batch_l.py /path/to/batches/batch1 [--first-only]

Same as run_batch_f.py but uses Luna (Inworld) voice and appends _l
to every session ID / filename. Output goes to Luna/batch_{first_id}_l/.
"""
...
OUTPUT_ROOT = Path.home() / "Desktop" / "overall_audio_batch" / "Luna"
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Batch meditation TTS — Luna voice (_l)")
    ...
    if args.output:
        ...
        batch_output = OUTPUT_ROOT / f"batch_{first_id}_l"
    ...
    for i, session in enumerate(sessions, 1):
        ...
        mp3_dest = batch_output / f"{sid_l}.mp3"
        if mp3_dest.exists():
            skipped.append(sid_l)
            print(f"\n[{i}/{len(sessions)}] {sid_l} — already exists, skipping")
            continue

        print(f"\n{'='*60}")
        print(f"[{i}/{len(sessions)}] {sid_l} — {title}")
        print(f"{'='*60}")

        temp_dir = batch_output / f"_temp_{sid_l}"
        temp_dir.mkdir(parents=True, exist_ok=True)

        try:
            generated = generate_meditation(
                sid_l, script, voice_id="Luna",
                output_dir=str(temp_dir), timestamps=True,
            )

            ts_dest = batch_output / f"{sid_l}_timestamps.json"
            shutil.move(str(generated["mp3"]), str(mp3_dest))
            shutil.move(str(generated["timestamps"]), str(ts_dest))

            backup_dir = batch_output / f"backup_{sid_l}"
            backup_dir.mkdir(parents=True, exist_ok=True)
            shutil.move(str(generated["calculated"]), str(backup_dir / f"{sid_l}_calculated.json"))
            shutil.move(str(generated["log"]), str(backup_dir / f"{sid_l}_session_log.md"))

            shutil.rmtree(str(temp_dir), ignore_errors=True)

            results.append({"id": sid_l, "title": title, "mp3": str(mp3_dest)})
            consecutive_errors = 0

        except Exception as e:
            errors.append({"id": sid_l, "title": title, "error": str(e)})
            print(f"\n❌ ERROR processing {sid_l}: {e}")
            shutil.rmtree(str(temp_dir), ignore_errors=True)
